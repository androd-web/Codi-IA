...
export const Questionnaire = ({ step, answers, updateAnswer, onNext, onBack }: QuestionnaireProps) => {
  ...
  const isNextDisabled = () => {
    ...
    if (q.type === 'multi') return !currentAnswer || (currentAnswer as string[]).length === 0;
    return false;
  };
  ...
  return (
    <div className="fade-in">
      ...
      <div className="bg-surface border border-muted h-1 rounded-sm mb-10 overflow-hidden">
        <div 
          className="h-full bg-gradient-to-r from-vert to-[#7fff6e] transition-all duration-500" 
          style={{ width: `${progress}%` }} 
        />
      </div>

      <div className="card">
        <div className="font-mono text-[10px] tracking-[2px] text-[#ff6b35] uppercase mb-3">
          {q.label}
        </div>
        ...
        <div id="input-area">
          {q.type === 'single' && (
            <div className="grid gap-2.5">
              {q.options?.map((o: string | Option, i: number) => {
                ...
                const iconName = typeof o === 'string' ? 'CircleDot' : (o.icon || 'CircleDot');
                ...
                return (
                  <button 
                    ...
                    className={cn("option-btn", selected && "selected")}
                  >
                    <span className="flex-shrink-0 mt-0.5 text-lg">
                      <Icon name={iconName} />
                    </span>
                    <span className="flex flex-col text-left">
                      <strong className="block font-bold mb-0.5 text-inherit">{label}</strong>
                      {desc && <span className="text-[12px] text-muted leading-relaxed">{desc}</span>}
                    </span>
                  </button>
                );
              })}
          ...
          {q.type === 'multi' && (
            <div className="flex flex-wrap gap-2">
              {q.options?.map((o: string | Option, i: number) => {
                const label = typeof o === 'string' ? o : o.label;
                const selected = ((currentAnswer as string[] | undefined) || []).includes(label);
                return (
                  <button 
                    key={i}
                    onClick={() => {
          ...
          {q.type === 'slider' && (
            <div className="py-2">
              ...
              <div className="flex justify-between font-mono text-[11px] text-muted">
                <span>{q.min}{q.unit?.split('/')[0]}</span>
                <span>{q.max}{q.unit?.split('/')[0]}</span>
              </div>
      ...
      <div className="flex justify-between items-center mt-8 gap-4">
        ...
        >
          {step === QUESTIONS.length - 1 ? (
            <>Générer ma feuille de route <LucideIcons.ArrowRight size={16} className="ml-1" /></>
          ) : (
